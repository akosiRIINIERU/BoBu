...
export default function Notificationll() {
  ...
  const handlePress = (notification) => {
    ...
    switch (notification.type) {
      case "chat":
        if (tenant) navigation.navigate("LandlordChat", { tenant });
        else alert("Tenant data not found");
        break;
      case "payment":
        if (tenant) navigation.navigate("Payments", { tenantId: tenant.id });
        else alert("Tenant data not found");
        break;
      default:
...
const styles = StyleSheet.create({
  ...
  text: { color: "#1D1D82", fontSize: 16, fontWeight: "600" },
});
